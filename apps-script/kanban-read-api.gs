...
function promoteRecord_(body) {
  ...
  try {
    ...
    if (handoff.status==='Promoted' && handoff.destinationId) {
      return json_({ok:true,source:'Occono TEST and DEVELOPMENT workflow workbooks',promotion:{handoffId:handoff.id,destinationId:handoff.destinationId,targetWorkflow:targetKey,record:findRecord_(targetKey,handoff.destinationId)}},200);
    }

    const destinationId = targetKey==='enquiries'
      ? createEnquiryFromOutreach_(id,sourceRecord)
      : createProjectFromEnquiry_(id,sourceRecord);
...
function createEnquiryFromOutreach_(sourceId, sourceRecord) {
  const sourceTable=rawTable_('outreach'), sourceRow=findRawRow_(sourceTable,sourceId);
  const targetTable=rawTable_('enquiries'), newId=nextId_(targetTable.rows.map(r=>r[targetTable.column['Enquiry ID']]),'ENQ');
  const values={
    'Enquiry ID':newId,
    'Created Timestamp':new Date(),
    'Source':`Outreach promotion: ${sourceId}`,
    'Name':value_(sourceRow,sourceTable.column,'Contact Name') || sourceRecord.contact || '',
    'Business':value_(sourceRow,sourceTable.column,'Business Name') || sourceRecord.title || '',
    'Email':value_(sourceRow,sourceTable.column,'Email'),
    'Phone':value_(sourceRow,sourceTable.column,'Phone'),
    'Website Type':'Not sure yet',
    'Guide Budget':'Not sure yet',
    'Original Message':promotionSummary_(sourceId,sourceRecord,value_(sourceRow,sourceTable.column,'Website URL')),
    'Status':'New',
    'Qualification':'Not assessed',
    'Approved Route':'Manual handling',
    'Owner':normaliseOwnerName_(sourceRecord.ownerName),
    'Next Action':'Review promoted outreach enquiry',
    'Automation Mode':'Manual',
    'Record Lock':'No',
    'Reminder Stage':`Promoted from ${sourceId}`,
  };
  appendMappedRow_(targetTable.sheet,targetTable.headers,values);
  return newId;
}
...
function createProjectFromEnquiry_(sourceId, sourceRecord) {
  ...
  const values={
    'Project ID':newId,
    'Project Name':sourceRecord.title || sourceId,
    'Business Name':sourceRecord.title || '',
    'Status':'Ready to start',
    'Current Phase':'Ready to start',
    'Owner':normaliseOwnerName_(sourceRecord.ownerName),
    'Start Gate':'Pending project setup',
    'Created Timestamp':new Date(),
    'Record Lock':'No',
    'Environment':'DEVELOPMENT',
    'Notes':`Promoted from accepted enquiry ${sourceId}. ${sourceRecord.summary || ''}`,
  };
  appendMappedRow_(targetTable.sheet,targetTable.headers,values);
  return newId;
}
...
function ensureHandoff_(spreadsheet,sourceWorkflow,sourceId,targetWorkflow,record,actorEmail) {
  ...
  const row={
    'Handoff ID':handoffId,'Created Timestamp':new Date(),'Source Workflow':sourceWorkflow,'Source Record ID':sourceId,
    'Target Workflow':targetWorkflow,'Business / Project':record.title||'','Contact':record.contact||'','Summary':record.summary||'',
    'Owner Name':record.ownerName||'','Owner Email':record.ownerEmail||'','Status':'Pending review','Actor Email':actorEmail,'Environment':CONFIG.environment
  };
  appendMappedRow_(sheet,required,row);
  return {id:handoffId,status:'Pending review',destinationId:'',rowNumber:sheet.getLastRow()};
}

function markHandoffPromoted_(spreadsheet,rowNumber,destinationId) {
  const sheet=spreadsheet.getSheetByName(CONFIG.handoffSheetName), headers=sheet.getRange(1,1,1,sheet.getLastColumn()).getDisplayValues()[0], column=columnMap_(headers);
  sheet.getRange(rowNumber,column['Status']+1).setValue('Promoted');
  sheet.getRange(rowNumber,column['Destination Record ID']+1).setValue(destinationId);
  sheet.getRange(rowNumber,column['Promoted Timestamp']+1).setValue(new Date());
}

function updateSourceLink_(workflow,sourceId,destinationId) {
  if (workflow!=='outreach') return;
  const table=rawTable_('outreach'), rowIndex=table.rows.findIndex(r=>r[table.column['Prospect ID']]===sourceId);
  if (rowIndex<0) return;
  if ('Enquiry Handoff Status' in table.column) table.sheet.getRange(rowIndex+2,table.column['Enquiry Handoff Status']+1).setValue('Promoted');
  if ('Enquiry ID' in table.column) table.sheet.getRange(rowIndex+2,table.column['Enquiry ID']+1).setValue(destinationId);
  if ('Updated Timestamp' in table.column) table.sheet.getRange(rowIndex+2,table.column['Updated Timestamp']+1).setValue(new Date());
}

function promotedSourceIds_() {
  const ids=new Set();
  ['enquiries','delivery'].forEach(key=>{
    const spreadsheet=SpreadsheetApp.openById(CONFIG.sources[key].spreadsheetId), sheet=spreadsheet.getSheetByName(CONFIG.handoffSheetName);
    if (!sheet || sheet.getLastRow()<2) return;
    const values=sheet.getDataRange().getDisplayValues(), column=columnMap_(values[0]);
    if (!('Status' in column) || !('Source Record ID' in column)) return;
    values.slice(1).forEach(row=>{if(row[column['Status']]==='Promoted') ids.add(row[column['Source Record ID']]);});
  });
  return ids;
}

function applyStatus_(workflow,sheet,rowNumber,column,status){
  if(workflow==='enquiries'){requireColumns_(column,['Status']);sheet.getRange(rowNumber,column['Status']+1).setValue(status);return;}
  if(workflow==='outreach'){requireColumns_(column,['Current Status']);const values={'Prospect found':'Prospect found','Qualified':'Qualified','Outreach ready':'Qualified — draft awaiting approval','Contacted':'Contacted — awaiting response','Follow-up':'Follow-up due','Engaged':'Engaged','Converted':'Converted'};sheet.getRange(rowNumber,column['Current Status']+1).setValue(values[status]);return;}
  requireColumns_(column,['Status','Current Phase']);const values={'Ready to start':['Ready to start','Ready to start'],'In progress':['Active','In progress'],'Waiting':['Active','Waiting'],'Review':['Active','Review'],'Ready to launch':['Active','Ready to launch'],'Complete':['Complete','Delivered']};const pair=values[status];sheet.getRange(rowNumber,column['Status']+1).setValue(pair[0]);sheet.getRange(rowNumber,column['Current Phase']+1).setValue(pair[1]);
}
...
function appendMappedRow_(sheet,headers,values){const row=headers.map(h=>Object.prototype.hasOwnProperty.call(values,h)?values[h]:'');sheet.appendRow(row);}
